...
class Map:
    ...
    def is_within_bounds(self, position):
        ...
        return (
            0 <= x_coordinate < self._width
            and 0 <= y_coordinate < self._height
        )
    ...
    def add_resource(self, resource):
        ...
        if self.is_occupied(resource.position):
            raise ValueError("Map position is already occupied")

        if resource.position in self._resources:
            raise ValueError("Map position already has a resource")

        self._resources[resource.position] = resource

    def remove_resource(self, resource):
        """Remove a collectible resource from the map."""
    ...
    def remove_unit(self, unit):
        ...
        if (
            unit.position not in self._units
            or self._units[unit.position] is not unit
        ):
    ...
    def move_unit(self, unit, destination):
        ...
        if distance > unit.movement_range:
            raise ValueError(
                "Destination is outside the unit's movement range"
            )
