...
class Game:
    ...
    def attack(self, player, attacker, target):
        ...
        if target_owner is None or target_owner is player:
            raise ValueError(
                "Target must belong to another registered player"
            )
        ...
        if (
            self._distance(attacker.position, target.position)
    ...
    def end_turn(self):
        ...
        if len(self._players) < 2:
            raise ValueError(
                "At least two players are required to end a turn"
            )

        if self.is_over:
            raise ValueError(
                "Cannot end the turn after the game is over"
            )
